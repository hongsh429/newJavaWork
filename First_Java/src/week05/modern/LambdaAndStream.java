...
public class LambdaAndStream {
    public static void main(String[] args) {
        ...
        // addAll : 모든 collection을 넣어라/
        //        parkingLot.addAll(parkingCarWithTicket(carsWantToPark));  /* 티겟잇으면 들어와 */
        parkingLot.addAll(parkCars(carsWantToPark, Car::hasTicket));

        //        parkingLot.addAll(parkingCarWithMoney(carsWantToPark));/* 티켓없어도 돈잇으면 들어와 */
        parkingLot.addAll(parkCars(carsWantToPark, Car::noTicketButMoney));

        // 익명함수 적용
        parkingLot.addAll(parkCars(carsWantToPark, (car) -> car.hasParkingTicket() && car.getParkingMoney() > 1000));
    ...
    /*
        함수를 매개 변수로 보내야하는데,, 타입은 뭐며 어떻게 보내지?
        Java에서는 함수형 인터페이스로 함수의 타입을 정의할 수 있으며, 그 구현체를 나중에 정의할 수 있다!
        함수형 인터페이스는 딱 1개의 함수만 가지고 있는 것!
